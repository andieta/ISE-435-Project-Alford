...
class MP3Play(kin.Frame):
    ...
    def buttons_fancyPrg(self):
        ...
        self.shuffleThis = kin.Button(self.buttons, bg="#FFE5B4", fg="black", font=16)
        self.shuffleThis["text"] = "shuffle"
        self.shuffleThis["command"]= self.shuffleThese #insert the shuffle fx 
        self.shuffleThis.grid(row=0, column=1)
    ...
    def goBackOne(self):
        if self.rightNow >0:
            ...
            self.rightNow=0
        self.boxy.itemconfigure(self.rightNow+1, bg="white")
        self.runItUp()
    
    
    def skipThisOne(self):
        if self.rightNow < len(self.musicList) - 1:
            self.rightNow +=1
        else:
            self.rightNow = 0
        self.boxy.itemconfigure(self.rightNow, bg="white")
